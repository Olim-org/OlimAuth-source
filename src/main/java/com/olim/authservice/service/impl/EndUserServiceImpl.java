...
import com.olim.authservice.exception.customexception.*;
...
//        String refresh = (String) redisTemplate.opsForValue().get(email);
    ...
    @Override
    public CheckPhoneResponse valPhone(String phone) {
        Optional<EndUser> endUser = this.endUserRepository.findByPhoneNumber(phone);
        if (endUser.isPresent()) {
            CheckPhoneResponse checkPhoneResponse = new CheckPhoneResponse("이미 이 번호가 사용되고 있습니다.", true);
            return checkPhoneResponse;
        }
        CheckPhoneResponse checkPhoneResponse = new CheckPhoneResponse("휴대폰 번호가 중복되지 않습니다.", false);
        return checkPhoneResponse;
    }
    ...
    private EndUser registerNewUser(RegisterDto registerDto, PasswordEncoder passwordEncoder, EndUserRepository endUserRepository) {
        ...
        EndUser endUser = EndUser.createUser(
                ...
                registerDto.address(),
                LocalDate.parse(registerDto.birthDate(), DateTimeFormatter.ISO_DATE),
                registerDto.gender(),
